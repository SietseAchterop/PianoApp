...
// Scheduler settings. Assuming data size not larger than 30 byte!
#define SCHED_MAX_EVENT_DATA_SIZE   30
...
// add timers
APP_TIMER_DEF(my_timer_id);                                                         /**< Handler for control loop */
...
/* Device name and passkey can  be set via a command
       last part of passkey and device name should match for simplicity!
*/
char serial[] = "104";
char name[] = "PianoApp_104";
uint8_t passkey[] = "100104";
...
static void pm_evt_handler(pm_evt_t const * p_evt)
{
	...
	NRF_LOG_INFO("Connecting failed, disconnecting: ");
	err_code = pm_peer_id_get(m_conn_handle, &m_peer_to_be_deleted);
	APP_ERROR_CHECK(err_code);
...
static void nus_data_handler(ble_nus_evt_t * p_evt)
{
  ...
  if (p_evt->type == BLE_NUS_EVT_RX_DATA)
	...
	strncpy(command, (char *)p_evt->params.rx_data.p_data, length);
    ...
    switch (p_ble_evt->header.evt_id)
    {
	    ...
	    */
        ...
        case BLE_GAP_EVT_LESC_DHKEY_REQUEST:
	     ...
	     NRF_LOG_INFO("-->    Dit is nummer %d", p_ble_evt->header.evt_id);
	     if (p_ble_evt->evt.gap_evt.params.auth_status.auth_status == 0x89) {
	       NRF_LOG_INFO("-->   auth_status == 0x89");
	       err_code = sd_ble_gap_disconnect(p_ble_evt->evt.gattc_evt.conn_handle,
						BLE_HCI_REMOTE_USER_TERMINATED_CONNECTION);
	       APP_ERROR_CHECK(err_code);
	     }
...
int main(void)
{
    ...
    fstore_init();
    mydata_init();
    // remove next line for random passkey's
    set_static_passkey();
